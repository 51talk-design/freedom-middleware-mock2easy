module.exports = function (mock2easy) {
  ...
  router.post('/load', function (req, res) {
     var routeUrl = req.body.url.replace(/&/g, '\/');
    if (routeUrl.lastIndexOf(".") == -1)
        routeUrl = routeUrl + ".json";
    else
        routeUrl = routeUrl.replace(global.options.interfaceSuffix, '.json');
  ...
  router.post('/del', function (req, res) {
    var routeUrl = req.body.interfaceUrl;
    if (routeUrl.lastIndexOf(".") == -1)
      routeUrl = req.body.interfaceUrl + ".json";
    else {
      if (global.options.interfaceSuffix)
        routeUrl = req.body.interfaceUrl.replace(global.options.interfaceSuffix, '.json');
    }
    fs.unlink(path.resolve(global.options.database) + routeUrl, function (err) {
      if (err) {
        mock2easy.error(err);
        res.send(500, err);
      } else {
