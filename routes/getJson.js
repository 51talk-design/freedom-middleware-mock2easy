module.exports = function (mock2easy, ignoreField) {
  ...
  return function (req, res, next) {
    ...
    fs.readFile(path.resolve(global.options.database) + routeUrl, 'utf-8', function (err, data) {
      if (err) {
        console.log(path.resolve(global.options.database) + routeUrl);
        mock2easy.log(_.template(global.language['SERVER-ERROR-LOG'])({ url: routeUrl }).red);
        res.send({ "code": "404", "message": _.template(global.language['SERVER-ERROR-LOG'])({ url: routeUrl }), "success": "false" });
      } else {
        if (data) {
